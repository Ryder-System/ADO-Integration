...
async function main() {
    ...
    try {
        ...
        // enrich the data based on references on the body
        vm = await enrichingValuesBasedOnBodyReferences(vm);
...
async function create(vm) {
    ...
    // if the work item parent id is no empty
    if (vm.env.parentWorkItemUrl != "") {
        patchDocument.push({
            op: "add",
            path: "/relations/-",
            value: {
                rel: "System.LinkTypes.Hierarchy-Reverse",
                url: vm.env.parentWorkItemUrl
            }
        });
    }
...
async function findWorkItem(vm, workItemId) {
    if (vm.env.logLevel >= 200) console.log(`Starting 'findWorkItem' method...`);
...
async  function enrichingValuesBasedOnBodyReferences(vm){
    ...
    if(matchResult != null &&  matchResult.length > 0){
        ...
        console.log("Getting the details of the Parent work item.");
